...
def generateEttusCommand():
    """chirp command generated from user-determined chirp values"""

    chirpCmd = ':::/home/vervet/LoWRES/radar '\
        + '--tx-rate ' + str(txfs) + 'e6 --rx-rate ' + str(rxfs) + 'e6 '\
        + '--tx-freq 0 --rx-freq 0 --chirp-bw '\
        + str(bw.get()) + ' --chirp-cf ' + str(cf.get()) + 'e6 --chirp-len '\
        + str(length.get()) + 'e-6 --chirp-amp ' + str(ampVolt) + ' --chirp-prf '\
        + str(prf.get()) + 'e3 --trace-len ' + str(traceLength.get()) + 'e-6 --stack '\
        + str(stack.get()) + ':::'
    chirpCmd = chirpCmd.encode(encoding="utf-8", errors="strict")
    return chirpCmd
...
def transmitAnimate():
    ...
    global maxNum, minNum, numReceiveSamps, serverSocket

    bufferMax = 65536 #65536 Byte maximum in a buffer divided by 4 (4 bytes per float) = 16384
    ...
    if (numReceiveSamps < (bufferMax // 4) and numReceiveSamps > 0):
        bufferMax = numReceiveSamps * 4 #the maximum amount of floats we will get per trace
    ...
    print('Buffer Max: ' + str(bufferMax) + ' Bytes')
    cmd = generateEttusCommand()
    ...
    # START ETTUS RECEIVE LISTENER
    
    serverSocket.listen(5)
    inputs = [serverSocket]
    outputs = []

    # print('Waiting for connection with ettus program...')
    # try:
    #     r, wr, ex = select.select(inputs, outputs, [])
    # except select.error:
    #     print('Connection Failed. Press STOP')
    #     transmitStop()
    #     return
    # else:
    ...
    while (transmitLoop and not exitFlag):
        ...
        data += conn.recv(bufferMax)
        #if not data:
        #    continue
        #print(data)
        if ((len(data) // 4) >= numReceiveSamps):
            responseSamps = struct.unpack(
                    str(numReceiveSamps) + 'f', data[0:((numReceiveSamps) * 4)])
            if len(responseSamps) != numReceiveSamps:
                print('Mismatch length')
            if ((len(data) // 4) > numReceiveSamps):
                #If data read part of next trace, leave it
                data = data[(numReceiveSamps) * 4: len(data)]
            else:
                data = B''
...
def transmitStop():
    ...
    if (transmitLoop):
        #serverSocket.close()
        socks = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try: socks.connect(server_address)
        except:
            print('WARNING: can\'t reconnect to socket for stop signal')
        msg = B'STP::::::'
        try: socks.send(msg)
        except:
            pass
...
def generateChirp(): 
    ...
    ampVolt = math.sqrt((10 ** (float(amp.get()) / 10)) * .001 * 50) * math.sqrt(2)
...
# Create a TCP/IP socket (as a client)
